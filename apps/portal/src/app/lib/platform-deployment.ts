...
export function getPortalBranding(env: EnvLike) {
  return {
    ...
    appDescription: trimToUndefined(env.NEXT_PUBLIC_APP_DESCRIPTION) ?? DEFAULT_PORTAL_DESCRIPTION,
    termsUrl: trimToUndefined(env.NEXT_PUBLIC_TERMS_URL),
    privacyUrl: trimToUndefined(env.NEXT_PUBLIC_PRIVACY_URL),
  };
}

export function getVisiblePortalAuthOptions(env: EnvLike): VisiblePortalAuthOptions {
  ...
  return {
    ...
    showMicrosoft:
      Boolean(env.AUTH_MICROSOFT_CLIENT_ID && env.AUTH_MICROSOFT_CLIENT_SECRET) &&
      (!enforcedProvider || enforcedProvider === 'microsoft'),
    allowOtp: !enforcedProvider,
  };
}

export function hasVisiblePortalAuthOptions(authOptions: VisiblePortalAuthOptions): boolean {
